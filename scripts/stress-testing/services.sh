...
# Register 100 different services with v1 tag in 9000 port range.
i=1
echo "[+] -- Registering 100 services"
while [[ i -lt 101 ]]; do
  port=$(( i + 9000 ))
  echo "++++ Registering service redis$i:$port"
  curl \
      --request PUT \
...
# Stress Consul's service registration by updating each instance's tag every 1s interval
k=100
while :
do
  i=1
